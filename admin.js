...
function getStoredAdmin() { return null; }
...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!authOk) {
      ...
      authOk = (email === DEFAULT_EMAIL && password === DEFAULT_PASS);
    }
    if (authOk) {
      setSession();
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (changePasswordBtn) {
    changePasswordBtn.addEventListener('click', () => {
      ...
      const val = String(newAdminPassword.value || '').trim();
      if (!val) { if (settingsMsg) settingsMsg.textContent = 'Password cannot be empty'; return; }
      try {
        fetch('/.netlify/functions/content', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ op: 'setadmin', email: 'lmahadev50@gmail.com', password: val })
        }).then(() => { if (settingsMsg) settingsMsg.textContent = 'Password updated'; })
          .catch(() => { if (settingsMsg) settingsMsg.textContent = 'Password update failed'; });
      } catch {
        if (settingsMsg) settingsMsg.textContent = 'Password update failed';
      }
